...
@note       Mar 2023, Modified to support multi byte read
            and general cleanup
...
*/
static void sd_spi_clear_buffer(void);
...
@return		An error code as defined by one of the SD_ERR_* definitions.
*/
static int8_t sd_spi_write_out_data(uint32_t block_address, void *data,
...
@return		An error code as defined by one of the SD_ERR_* definitions.
*/
static int8_t sd_spi_read_block(uint32_t block_address, void *data_buffer);
...
*/
static uint8_t sd_spi_send_byte_command(uint8_t command, uint32_t argument);
...
@return	The R1 response from the card.
*/
static uint8_t spi_send_byte_app_command(uint8_t command, uint32_t argument);
...
*/
static int8_t sd_spi_wait_if_busy(uint32_t max_time_to_wait);
...
@return	An error code as defined by one of the SD_ERR_* definitions.
*/
static int8_t sd_spi_r1_error(uint8_t r1_response);
...
@return	An error code as defined by one of the SD_ERR_* definitions.
*/
static int8_t sd_spi_r2_error(uint16_t r2_response);

/**
@brief	Asserts the chip select pin for the card.
*/
static void sd_spi_select_card(void);

/**
@brief	De-asserts the chip select pin for the card.
*/
static void sd_spi_unselect_card(void);

const uint8_t tenFFBytes[10] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
int8_t sd_spi_init(void)
{
    ...
    /* Send at least 74 clock pulses to enter the native operating mode
       (80 in this case). */
    sd_spi_send_bytes(tenFFBytes, sizeof(tenFFBytes));
    ...
    while (sd_spi_send_byte_command(SD_CMD_GO_IDLE_STATE, 0) !=
           SD_IN_IDLE_STATE) {
    ...
    if ((sd_spi_send_byte_command(SD_CMD_SEND_IF_COND, 0x1AA) &
         SD_ILLEGAL_COMMAND) == 0) {
    ...
    if (card.card_type == SD_CARD_TYPE_SD2) {
        while (spi_send_byte_app_command(SD_ACMD_SEND_OP_COND, 0x40000000) !=
               0) {
    ...
    if (sd_spi_send_byte_command(SD_CMD_SET_BLOCKLEN, SD_BLOCK_SIZE)) {
        sd_spi_unselect_card();
...
int8_t sd_spi_write(uint32_t block_address, void *data,
                    uint16_t number_of_bytes, uint16_t byte_offset)
{
    /* Check to make sure that data is within page bounds. */
    if (number_of_bytes + byte_offset > SD_BLOCK_SIZE) {
    ...
    if (number_of_bytes == SD_BLOCK_SIZE && !card.is_read_write_continuous) {
        int8_t response = sd_spi_write_block(block_address, data);
        sd_spi_unselect_card();

        return response;
    } else if (!card.is_read_write_continuous &&
        ...
        if (sd_spi_dirty_write) {
            ...
            if ((response =
                     sd_spi_read_block(block_address, card.sd_spi_buffer))) {
                return response;
            }
        }
    }
...
int8_t sd_spi_write_block(uint32_t block_address, void *data)
{
    int8_t response;
    ...
    response = sd_spi_write_out_data(block_address, data, SD_BLOCK_SIZE, 0);

    if (!card.is_read_write_continuous ||
        block_address == card.continuous_block_address) // TODO: Does this logic
                                                        // make sense?
    {
        memcpy(card.sd_spi_buffer, data, SD_BLOCK_SIZE);
    }
...
int8_t sd_spi_flush(void)
{
    ...
    if ((response = sd_spi_write_out_data(card.buffered_block_address,
                                          card.sd_spi_buffer, SD_BLOCK_SIZE,
                                          0))) {
        return response;
    }
...
int8_t sd_spi_write_continuous_start(uint32_t start_block_address,
                                     uint32_t num_blocks_pre_erase)
{
    ...
    if (card.card_type != SD_CARD_TYPE_SDHC) {
        start_block_address *= SD_BLOCK_SIZE;
    }
    ...
    if (sd_spi_send_byte_command(SD_CMD_WRITE_MULTIPLE_BLOCK,
                                 start_block_address)) {
        sd_spi_unselect_card();
...
int8_t sd_spi_write_continuous(void *data, uint16_t number_of_bytes,
                               uint16_t byte_offset)
{
...
int8_t sd_spi_write_continuous_next(void)
{
...
int8_t sd_spi_write_continuous_stop(void)
{
...
int8_t sd_spi_read(uint32_t block_address, void *data_buffer,
                   uint16_t number_of_bytes, uint16_t byte_offset)
{
    int8_t response;
    ...
    if (card.buffered_block_address != block_address ||
        ...
        if ((response = sd_spi_read_block(block_address, card.sd_spi_buffer))) {
            return response;
        }
    }
...
int8_t sd_spi_read_continuous_start(uint32_t start_block_address)
{
    int8_t response;
    ...
    if (sd_spi_send_byte_command(SD_CMD_READ_MULTIPLE_BLOCK,
                                 start_block_address)) {
        sd_spi_unselect_card();
    ...
    if ((response = sd_spi_read_block(card.continuous_block_address,
                                      card.sd_spi_buffer))) {
        sd_spi_unselect_card();
        return response;
    }
...
int8_t sd_spi_read_continuous(void *data_buffer, uint16_t number_of_bytes,
                              uint16_t byte_offset)
{
...
int8_t sd_spi_read_continuous_next(void)
{
    return sd_spi_read_block(card.continuous_block_address, card.sd_spi_buffer);
}

int8_t sd_spi_read_continuous_stop(void)
{
    sd_spi_select_card();
...
int8_t sd_spi_erase_all(void)
{
    return sd_spi_erase_blocks(0, sd_spi_card_size());
}

int8_t sd_spi_erase_blocks(uint32_t start_block_address,
...
uint32_t sd_spi_card_size(void)
{
    ...
    if (csd_structure == 0) {
        ...
        number_of_blocks = (uint32_t)((c_size_high << 8 | c_size_low) + 1) *
                           (1 << (c_size_mult + 2));
        number_of_blocks *= (uint32_t)(1 << max_read_bl_len) / SD_BLOCK_SIZE;
    } else {
        b = sd_spi_receive_byte();
        c_size_high = b;
        b = sd_spi_receive_byte();
        ...
        number_of_blocks =
            (uint32_t)(c_size_high << 16 | c_size_mid << 8 | c_size_low) + 1;
        number_of_blocks <<= 10;
    }
...
int8_t sd_spi_read_cid_register(sd_spi_cid_t *cid)
{
...
int8_t sd_spi_read_csd_register(sd_spi_csd_t *csd)
{
    if (sd_spi_send_byte_command(SD_CMD_SEND_CSD, 0)) {
        sd_spi_unselect_card();
        return SD_ERR_READ_REGISTER;
...
int8_t sd_spi_card_status(void)
{
    int8_t response =
        sd_spi_r2_error((sd_spi_send_byte_command(SD_CMD_SEND_STATUS, 0) << 8) |
                        sd_spi_receive_byte());
...
uint8_t sd_spi_card_type(void)
{
    return card.card_type;
}

uint32_t sd_spi_current_buffered_block(void)
{
                                    ...
                                    uint16_t byte_offset)
{
    ...
    /* Pad data with 0. */
    for (i = byte_offset + number_of_bytes; i < SD_BLOCK_SIZE; i++) {
        sd_spi_send_byte(0);
    }
...
static int8_t sd_spi_read_block(uint32_t block_address, void *data_buffer)
{
    ...
    /* Read block into sd_spi_buffer if it is defined. \
     */
    /* Read in the bytes to the buffer. */
    sd_spi_receive_bytes(data_buffer, SD_BLOCK_SIZE);
    ...
    if (card.is_read_write_continuous) {
        card.continuous_block_address++;
        // TODO: HACK
        if (data_buffer == card.sd_spi_buffer)
            card.buffered_block_address = card.continuous_block_address;
    } else {
        if (card.card_type != SD_CARD_TYPE_SDHC) {
            block_address >>= 9;
...
static uint8_t sd_spi_send_byte_command(uint8_t command, uint32_t argument)
{
    ...
    uint8_t command_buffer[6] = {
        0x40 | command, // Command Byte
        (argument >> 24U) & 0xFF, // ARG MSB
        (argument >> 16U) & 0xFF, // ARG
        (argument >> 8U) & 0xFF, // ARG
        (argument >> 0U) & 0xFF, // ARG LSB
        0xFF, // To Be Filled CheckSum
    };
    ...
    case SD_CMD_GO_IDLE_STATE:
        command_buffer[5] = 0x95;
        break;
    /* CRC for CMD8 with arg 0X1AA. */
    case SD_CMD_SEND_IF_COND:
        command_buffer[5] = 0x87;
        break;
    default:
        break;
    }
    sd_spi_send_bytes(command_buffer, sizeof(command_buffer));
