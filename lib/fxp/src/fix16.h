...
static inline int fix16_same_sign(fix16_t a, fix16_t b)
{
  if (a >= 0)
    return b >= 0;
  else
...
static inline fix16_t fix16_copy_sign(fix16_t dst, fix16_t src)
{
    if (src >= 0) {
        if (dst >= 0) {
            return dst;
...
#else
int32_t fix16_mul_int32(fix16_t a, int32_t b);
int32_t fix16_mul_int(fix16_t a, fix16_t b);
